...
def SelectPointsNearGround(points, bv_common_settings):
    ...
    points1 = points[(p_h >= shifted_min_height) & (p_h <= shifted_max_height), :] #选取处于min_height与max_height之间的点

    return points1


def AdjustIntensity(points, bv_common_settings):  #强度归一化
...
''' get the class of each point for each lidar id
    input: 
        projected to real world points;
        bv label map;
        bv settings
        bv range settings

    output:
        dictionary: for each lidar id export each points' class ([n, 1] array)
