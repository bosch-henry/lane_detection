...
def WriteLineFittingResultToFile(matrix, filename):
    _, dim = matrix.shape
    if dim == 3:
        f = pd.DataFrame(matrix, columns=["X", "Y", "Y_Pred"])
        f.to_csv(filename, index=False)
    elif dim == 1:
        f = pd.DataFrame(matrix, columns=["coef"])
        f.to_csv(filename, index=False)
...
def GetLineScatterDataList(folder, side_list):
    data_num = len(glob.glob(os.path.join(folder, "line_side_id", side_list[0], "*.csv")))
    scatter_name_set_list = []
    for j in range(data_num):
        scatter_name_set_list.append(dict())
    
    for line_side in side_list:
        scatter_list_temp = glob.glob(os.path.join(folder, "line_side_id", line_side, "*.csv"))
        scatter_list_temp.sort()
        
        for j in range(data_num):
            scatter_name_set_list[j][line_side] = scatter_list_temp[j]

    return scatter_name_set_list
...
def ReadLineScatterPoints(file_name_set):
    line_side_list = file_name_set.keys()
    scatter_result = {}

    for line_side in line_side_list:
        scatter_file_name = file_name_set[line_side]
        scatter_points = ReadCsvFile(scatter_file_name)
        scatter_result[line_side] = scatter_points
...
def OutputLineFittingResult(line_scatter_pred_matrix, line_coef, line_side_set, output_path):
    line_side_list = line_side_set.keys()
    output_path_pred_matrix = os.path.join(output_path, "pred_matrix")
    output_path_coef = os.path.join(output_path, "coef")
    if not os.path.exists(output_path_pred_matrix):
        os.mkdir(output_path_pred_matrix)
    if not os.path.exists(output_path_coef):
        os.mkdir(output_path_coef)

    for line_side in line_side_list:
        output_folder_pred_matrix = os.path.join(output_path_pred_matrix, line_side)
        output_folder_coef = os.path.join(output_path_coef, line_side)
        if not os.path.exists(output_folder_pred_matrix):
            os.mkdir(output_folder_pred_matrix)
        if not os.path.exists(output_folder_coef):
            os.mkdir(output_folder_coef)

        line_name0 = line_side_set[line_side]
        pure_name = line_name0.split("/")[-1]
        output_name_pred_matrix = os.path.join(output_folder_pred_matrix, pure_name)
        output_name_coef = os.path.join(output_folder_coef, pure_name)

        WriteLineFittingResultToFile(line_scatter_pred_matrix[line_side], output_name_pred_matrix)
        WriteLineFittingResultToFile(line_coef[line_side], output_name_coef)
